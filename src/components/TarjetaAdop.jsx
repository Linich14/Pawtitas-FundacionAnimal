...
const VerificarSolicitud = ({ solicitudId }) => {
  ...
  useEffect(() => {
    ...
    const obtenerDatosSolicitud = async () => {
      ...
      const solicitudDocRef = doc(db, 'SolicitudesDarAdopcion', solicitudId);
      ...
      const subcoleccionesRef = collection(solicitudDocRef, 'SolicitudesDeAdopcion');
  ...
  const aceptarSolicitud = async () => {
    try {
      ...
      const subcoleccionesRef = collection(db, 'SolicitudesDarAdopcion', solicitudId, 'SolicitudesDeAdopcion');
      const subcoleccionesQuery = await getDocs(subcoleccionesRef);
      ...
      const solicitudAdopcionRef = doc(db, 'SolicitudesDarAdopcion', solicitudId);
  ...
  const rechazarSolicitud = async (subcoleccionId) => {
    try {
      const subcoleccionRef = doc(db, 'SolicitudesDarAdopcion', solicitudId, 'SolicitudesDeAdopcion', subcoleccionId);
  ...
  return (
    ...
    {solicitudData && (
      <div>
        <h2 className="solicitudDe">Solicitud de dar en adopcion </h2>
        {subcoleccionesData.map((subcoleccion) => (
          <div className="tarjetaSoliContenido" key={subcoleccion.id}>
            <div className="tarjetaContenido">
              ...
              <p>nombre: {subcoleccion.data.Animal_Nombre}</p>
              <p>raza: {subcoleccion.data.Animal_Raza}</p>
